...
+ `scale_html`: 
  ...
  In the process of converting DVI to SVG, it appears that the figure gets shrunk (as it appears on my screen in the web brower) unless we apply a scale > 1. 
  This scale is applied as an argument to the utility `dvisvgm` that we use for DVI to SVG conversion.
  The value 1.28 has been empirically determined so that the text in the image matches in size to the surrounding body text in the HTML. 
  It may differ for other screens. 
