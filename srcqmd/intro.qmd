...
This is a pandoc span with the class name `ar`. I have written a pandoc filter in `_extensions/arabic_support/inline-arabic-span.lua` that adds the correct attributes (`lang="ar"` and `dir="rtl"`) to the span so that it is processed correctly in the output:
...
Another custom pandoc filter (`_extensions/arabic_support/transliteration-span.lua`) processes the above ASCII and adds the Unicode dots, macrons, and other diacritics to render it correctly.
